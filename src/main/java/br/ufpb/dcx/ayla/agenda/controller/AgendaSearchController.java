...
public class AgendaSearchController implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(aniversariantes.size()>0){
            JOptionPane.showMessageDialog(janelaPrincipal,"Aniversaiante encontrados");
            for(Contato c: aniversariantes){
                JOptionPane.showMessageDialog(janelaPrincipal,c.toString());
            }
        }
            else{
                JOptionPane.showMessageDialog(janelaPrincipal,"Nao foi encontrado nenhum aniversariante nesta data");
            }
        }
